  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-1 text-xs text-emerald-600 dark:text-emerald-400 font-medium">
              ...
              <span>{todaySales.length} successful receipts</span>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">7 Days Revenue</span>
            <div className="flex items-baseline gap-1">
            ...
            <div className="flex items-center gap-1 text-xs text-sky-600 dark:text-sky-400 font-medium">
              ...
              <span>Active flow tracking</span>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Gross Profit (Profit %)</span>
            <div className="flex items-baseline gap-1">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">
              Avg. Margin: <strong className="text-slate-700 dark:text-slate-200">{totalRevenue > 0 ? Math.round((totalProfit / totalRevenue) * 100) : 0}%</strong>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-start justify-between">
          <div className="space-y-2">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">
              Net: <strong className="text-slate-800 dark:text-slate-350">{currency} {netProfit.toLocaleString()}</strong>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            <div className="text-[10px] uppercase font-bold text-slate-400">Total Catalog</div>
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-base font-bold text-rose-600 dark:text-rose-450">{lowStockProducts.length} Items</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-800 dark:text-white text-base">Weekly Billing Flow</h3>
              <p className="text-xs text-slate-400">Comparing gross billing against profit vectors</p>
            </div>
            <div className="flex items-center gap-3 text-xs">
              <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-indigo-500"></span>Gross Revenue</span>
                ...
                return (
                  <div key={index} className="flex flex-col items-center group relative w-12">
                    ...
                    <div className="absolute bottom-full mb-2 bg-slate-900 border border-slate-800 text-slate-100 text-[10px] p-2 rounded-xl opacity-0 group-hover:opacity-100 pointer-events-none transition duration-150 z-20 shadow-xl whitespace-nowrap">
                      <div>Revenue: <strong className="text-indigo-300">{currency} {d.sales.toLocaleString()}</strong></div>
                      <div>Profit: <strong className="text-emerald-300">{currency} {d.profit.toLocaleString()}</strong></div>
                    </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
          <div>
            <h3 className="font-bold text-slate-800 dark:text-white text-base">Best Selling Products</h3>
            <p className="text-xs text-slate-400">Highest volume retail items (Last 30 days)</p>
          </div>
                ...
                <div key={idx} className="flex items-center justify-between gap-2 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-850 hover:bg-slate-100/70 dark:hover:bg-slate-750 transition duration-150">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-[10px] text-slate-400">{p.qty} units shipped</div>
                    </div>
                  </div>
          ...
          {bestSellingProducts.length > 0 && (
            ...
            >
              Generate Complete Shipping Reports →
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-800 dark:text-white text-base">Critical Stock Alerts</h3>
              <p className="text-xs text-rose-500">Products nearing depleted quantities</p>
            </div>
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700 text-sm max-h-[260px] overflow-y-auto pr-1">
            {lowStockProducts.length > 0 ? (
              lowStockProducts.map((p) => (
                <div key={p.id} className="py-2.5 flex items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-slate-400">SKU: {p.sku} | Unit: {p.unit}</div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs">
                      Qty: <strong className={p.quantity <= 0 ? "text-rose-600 font-bold" : "text-amber-500 font-bold"}>
                        {p.quantity}
              ...
              <div className="text-center py-16 text-slate-400 text-xs flex flex-col items-center gap-2">
                ...
                All products are sufficiently stocked. Great!
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-bold text-slate-800 dark:text-white text-base">Local Audit Log</h3>
              <p className="text-xs text-slate-400">Continuous background operations logged locally</p>
            </div>
            <button
